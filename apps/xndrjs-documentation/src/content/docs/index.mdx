...
hero:
  ...
  actions:
    ...
    - text: Build the first model
      link: /latest/getting-started/first-model/
      icon: open-book
      variant: secondary
    - text: Read the blog
      link: /blog/
...
<CardGrid stagger>
  ...
  <Card title="Meaningful boundaries" icon="puzzle">
    ...
    - behavior -> how data evolves (`capabilities.forShape`, `capabilities.forPrimitive`)
  ...
  <Card title="Beyond the domain" icon="open-book">
    `xndrjs` is more than a set of domain utilities. It aims to define a shape-driven workflow for Clean Architecture: name the data at every boundary first, then let business logic and infrastructure code follow from those shapes.

    The domain is the most fundamental layer, but not the only one. Transport payloads, use-case inputs, view models, and async tasks all cross boundaries that deserve an explicit contract.

    The toolkit stays intentionally light on frameworks and infrastructure, while giving you primitives and a clear order of work — model the shape, map between layers, then write behavior.
...
1. Read the [introduction](/latest/getting-started/introduction/) to see what `xndrjs` is and what it deliberately is not.
2. Read the [mental model](/latest/getting-started/mental-model/) to understand how `xndrjs` thinks about trust boundaries.
3. Build the [first model](/latest/getting-started/first-model/) to see primitive, shape, capability, and proof working together.
4. Pick an adapter with [Choose an adapter](/latest/getting-started/choosing-adapter/).
5. Use the [Domain overview](/latest/domain/overview/), [Application resources](/latest/application/application-resources/), and [Tasks](/latest/infrastructure/tasks/) to place the packages inside a Clean Architecture project.

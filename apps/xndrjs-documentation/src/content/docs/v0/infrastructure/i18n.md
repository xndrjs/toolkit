...
Pass the shared `i18n` instance from your app bootstrap — the helper only registers missing namespaces via `setNamespace()`. Load a subset when a route needs fewer namespaces: `ensureNamespacesLoadedForLocale(i18n, activeLocale, ["billing"])`.
...
| `dictionary.generated.ts`        | Imports JSON and exports `defaultDictionary` (canonical) or `defaultDictionaryFor` when eager namespaces exist in split/custom delivery; otherwise `export {}` |

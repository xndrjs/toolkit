...
| `dictionary.generated.ts`        | Imports JSON (source or YAML-compiled) and exports `defaultDictionary`                                       |
...
List namespaces needed at startup in `loadOnInit`. Codegen emits typed `namespaceLoaders` — one dynamic `import()` per lazy namespace. `.get()` stays synchronous — register with `setNamespace()` before rendering.

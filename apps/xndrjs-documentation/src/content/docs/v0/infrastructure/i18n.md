...
`projectLocales(dictionary, locales, localeFallback?)` returns a dictionary with only the listed locales. Missing direct entries are filled via `localeFallback` (same rules as `.get()`), then you pass the result to `setAll()` or `setNamespace()`.
...
Codegen emits `projectLocales` next to `createI18n()` in `instance.generated.ts`: it injects `LOCALE_FALLBACK` and types locales as `MyProjectLocale[]`. Keys that cannot be resolved for a requested locale are omitted.

Common deployment patterns:

- **API** — resolve `projectLocales(fullDict, [requestedLocale], LOCALE_FALLBACK)` per request and return JSON.

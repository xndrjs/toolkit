...
For motivation, design trade-offs, and split-by-locale delivery, see [Type-safe i18n and flexible delivery](/blog/type-safe-i18n-with-runtime-overrides/).
...
With `"delivery": "split-by-locale"`, YAML and JSON sources compile to **per-locale** files instead (`billing.en.json`, `billing.it.json`, …). See [Split-by-locale delivery](#split-by-locale-delivery).
...
| `"split-by-locale"`     | One JSON per namespace **and locale**: `{basename}.{locale}.json`. Exports `defaultDictionaryFor(locale)`. Lazy loaders: `namespaceLoaders.billing(locale)`.                  |

Authoring stays on the canonical source files. See [Type-safe i18n and flexible delivery](/blog/type-safe-i18n-with-runtime-overrides/) for how authoring, compiler, delivery, and runtime stay separated. `MyProjectSchema` is generated explicitly (`Partial<Record<MyProjectLocale, string>>` per key from ICU analysis) — not from `typeof import` of JSON/YAML files. Audit still reads the config paths.
...
import { defaultDictionaryFor } from "./i18n/generated/dictionary.generated.js";
import { namespaceLoaders } from "./i18n/generated/namespace-loaders.generated.js";

const activeLocale = "it" as const;

export const i18n = createI18n(defaultDictionaryFor(activeLocale));
...
`createI18n` stays synchronous — pass the eager slice for the active locale via `defaultDictionaryFor`. Runtime `projectDictionaryLocales` / `projectNamespaceLocales` remain useful for external CMS/API payloads that still arrive as multilocale dictionaries.
...
| `dictionary.generated.ts`        | Imports JSON (source or YAML-compiled) and exports `defaultDictionary` (canonical) or `defaultDictionaryFor(locale)` (split-by-locale) |
...
| `namespace-loaders.generated.ts` | Optional — typed `namespaceLoaders` map (`import()` per lazy namespace)                                                                |
...
List namespaces needed at startup in `loadOnInit`. Codegen emits typed `namespaceLoaders` — one dynamic `import()` per lazy namespace. With `delivery: "split-by-locale"`, each loader is nested by locale. `.get()` stays synchronous — register with `setNamespace()` before rendering.
...
| `delivery`                          | Optional. `"canonical"` (default) or `"split-by-locale"`. See [Split-by-locale delivery](#split-by-locale-delivery).             |
...
- [Type-safe i18n and flexible delivery](/blog/type-safe-i18n-with-runtime-overrides/) — motivation, design, and delivery

...
Pass the shared `i18n` instance from your app bootstrap — the helper loads or updates namespaces via `mergeNamespace()` (locale entries accumulate when switching locale or area). Load a subset when a route needs fewer namespaces: `ensureNamespacesLoadedForLocale(i18n, activeLocale, ["billing"])`.
...
| `.mergeNamespace(ns, values)` | Multi only — merge locale entries per key (generated `ensureNamespacesLoaded*` in split/custom) |

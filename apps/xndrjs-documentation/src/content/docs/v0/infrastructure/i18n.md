...
Pass the shared `i18n` instance from your app bootstrap — the helper loads or updates translations via `mergeAll()` (single) or `mergeNamespace()` (multi); locale entries accumulate when switching locale or area. Load a subset when a route needs fewer namespaces: `ensureNamespacesLoadedForLocale(i18n, activeLocale, ["billing"])`.
...
| `.mergeAll(values)`           | Merge locale entries per key (single: whole schema; multi: each namespace). Alternative to `setAll` when accumulating locales |
| `.setNamespace(ns, values)`   | Multi only — replace one namespace; invalidate its cache entries                                                              |
| `.mergeNamespace(ns, values)` | Multi only — merge locale entries per key (generated `ensureNamespacesLoaded*` in multi split/custom)                         |

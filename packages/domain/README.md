...
**`domain`** groups the main factories: `shape`, `primitive`, `proof`, `capabilities`. Also exported at the package root: `compose`, `pipe`, and `DomainValidationError`. Types (`ShapeKit`, `Branded`, `Validator`, …) are exported at the root. Implementation details (`getShapePatchImpl`, `__brand`, `__patchImpl`, `__shapeMarker`) stay internal to the package.

Use a pluggable `Validator` (e.g. via [@xndrjs/domain-zod](../domain-zod) `zodToValidator`) at boundaries.

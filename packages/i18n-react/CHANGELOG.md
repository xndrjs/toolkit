...
- 99b2b07: **@xndrjs/i18n-react:** Fix `withI18n` Rules of Hooks violation when the load gate starts in `pending` (for example `I18nRoot` without hydrated `state`) and later becomes `ready`.
...
- 009c866: Ship compiled CLIs (`dist/cli/*`) instead of `tsx` launcher shims; remove the `tsx` peer dependency from both packages.
...
- Updated dependencies [009c866]
  - @xndrjs/i18n@0.8.2-alpha.0
